...
def get_qab(img_ab, sigma=5, bins=313):
    ...
    a = K.ravel(img_ab[:, :, 0])
    b = K.ravel(img_ab[:, :, 1])
    ab = K.vstack((a, b)).T  # now dim (H*W*2)
    distances, idx = nn.kneighbors(ab)

    # smoothen
    gaussian = K.exp(-distances**2 / (2*sigma**2))
    gaussian /= K.sum(gaussian, axis=1)[:, np.newaxis]
    soft_encoding = K.zeros((ab.shape[0], bins))
    pts = K.arange(ab.shape[0])[:, np.newaxis]
    soft_encoding[pts, idx] = gaussian
    return soft_encoding.reshape(h, w, bins)


def annealed_mean(im_qab):
    return 0


def get_data(path, size=64):
    img = resize_img(load_img(path), size)
    img_gray = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY).reshape((64, 64, 1))
    img_ab = get_ab(img)
